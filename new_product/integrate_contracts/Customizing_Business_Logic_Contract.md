<h1 align="center">Develop to Customize Contracts</h1>
...
Business contracts need to maintain a two mapping relationships between chains, one is the **asset mapping relationship**, and the other is the **business logic contract mapping relationship**.
...
- By calling this method, the business logic contract will **lock** a certain amount of valuable assets. And a `CrossChainEvent` will be emitted in [CCM contract](https://github.com/polynetwork/eth-contracts/blob/master/contracts/core/cross_chain_manager/interface/IEthCrossChainManager.sol) in order to be cought by the relayer in order to complete the remaining processes.
